...
export default function Reservation() {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault()
    setStatus('sending')
    /* In production: integrate EmailJS here with service/template IDs */
    await new Promise(r => setTimeout(r, 1800))
    setStatus('success')
    setTimeout(() => { setForm(INITIAL); setStatus('idle') }, 4000)
  }

  const inputClass = 'w-full bg-brand-dark-card border border-brand-dark-border text-brand-text-light font-inter text-sm px-4 py-3.5 focus:outline-none focus:border-brand-orange transition-colors duration-300 placeholder:text-brand-text-muted/50'

  return (
    <section ref={sectionRef} className="relative bg-brand-black py-24 md:py-36 overflow-hidden" id="reserve">
      {/* Ambient */}
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: 'radial-gradient(ellipse 55% 60% at 5% 50%, rgba(247,148,29,0.07) 0%, transparent 55%)' }}
      />
      ...
      <div className="container-site">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="res-left">
            ...
            <h2 className="font-playfair text-brand-cream leading-[1.05] mb-6"
              style={{ fontSize: 'clamp(2.2rem, 4.5vw, 4rem)' }}>
              Reserve Your <br />
              <span className="italic text-brand-orange">Table Today</span>
            </h2>
            <p className="font-cormorant text-brand-text-muted text-xl leading-relaxed italic mb-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                ...
                <div key={label} className="border border-brand-dark-border p-5 flex flex-col gap-2">
                  <Icon size={18} className="text-brand-orange" />
                  <p className="font-inter text-sm text-brand-cream font-medium">{label}</p>
                  <p className="font-inter text-[11px] text-brand-text-muted">{sub}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="res-right">
            <div className="border border-brand-dark-border p-8 md:p-10 relative">
              {/* Corner marks */}
              ...
              <AnimatePresence mode="wait">
                {status === 'success' ? (
                  ...
                  >
                    ...
                    <h3 className="font-playfair text-2xl text-brand-cream">Reservation Received</h3>
                    <p className="font-inter text-sm text-brand-text-muted max-w-xs">
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <input className={inputClass} type="date" required value={form.date} onChange={update('date')}
                        min={new Date().toISOString().split('T')[0]} />
                      <select className={inputClass} required value={form.time} onChange={update('time')}>
                        <option value="" disabled>Select Time</option>
                        {TIME_SLOTS.map(t => <option key={t} value={t}>{t}</option>)}
                      </select>
                    </div>

                    <select className={inputClass} value={form.guests} onChange={update('guests')}>
                    ...
                    <p className="font-inter text-[10px] text-brand-text-muted text-center leading-snug">
